...
source(system.file("shinyApp", "includeRmd.r", package="MMPde"))

addResourcePath(prefix = 'pics', directoryPath = system.file("shinyApp/www", package="MMPde"))
...
server <- function(input, output){
  ...
  load(system.file("shinyApp/data/", "mmp_data.RData", package = "MMPde"))
                                  ...
                                  if(input$ind_const == "TSS" & input$ind_plot_type1 == "Power Curves")
                                    ...
                                    if(input$ind_const == "TSS" & input$ind_plot_type1 == "Comparison (Pre/Post)")
                                      ...
                                      if(input$ind_plot_type1 == "Time to Exceedance"){
                                        ...
                                        tags$div(
                                          tags$img(src=paste0("pics/", fig_nm), contentType = 'image/png', height = 500),
                                          tags$script(src="titlescript.js")
                                        )
                                      }
                                      else{
                                        tags$div(
                                          tags$img(src=paste0("pics/", fig_nm), contentType = 'image/png', height = 400),
                                          tags$script(src="titlescript.js")
                                        )
                                      }
  })
  ...
  #------------------------------- Second Tab: Constituent Comparison (by Region) ---------------------------------#
    ...
    dat_img <- apply(dat, 1, function(x){
      sapply(x, function(y){
          if(y != ""){
              paste0("<img src='pics/", y, "' contentType = 'image/png' height = 400 ></img>")
          }
          else
            paste("")
        }
  ...
  #--------------------- Documentation ----------------------------------------#
    ...
    output$userguide <- renderUI({
      inclRmd(system.file("shinyApp/rmd", "userguide.Rmd", package="MMPde"))
    })

    output$ack <- renderUI({
      inclRmd(system.file("shinyApp/rmd", "ack.Rmd", package="MMPde"))
    })
    
    output$wqparms <- renderUI({
      inclRmd(system.file("shinyApp/rmd", "WQdesc.Rmd", package="MMPde"))
    })
    
    output$figdesc <- renderUI({
      inclRmd(system.file("shinyApp/rmd", "FigureDesc.Rmd", package="MMPde"))
    })
